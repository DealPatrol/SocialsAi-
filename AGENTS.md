...
- **Optional env** (features degrade gracefully without them): `ANTHROPIC_API_KEY` (post generation), `TWITTER_CLIENT_ID`/`TWITTER_CLIENT_SECRET` (log in + post to a real X account), `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`/`SUPABASE_SERVICE_ROLE_KEY` and `CRON_SECRET` (Automation tab + cron endpoints).
